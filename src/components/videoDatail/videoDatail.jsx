    ...
    const getData = async () => {
      try {
        const data = await ApiService.Feching(
          `videos?part=snippet,statistics&id=${VideoId.id}`
        );
        setVideoDatail(data.items);
        const reletedData = await ApiService.Feching(
          `search?part=snippet&relatedToVideoId=${VideoId.id}&type=video`
        );
            ...
            return (
              <div key={i} className="2xl:w-[1100px] xl:w-[900px] lg:w-[650px]">
                ...
                <div className="max-sm:text-[12px]">
                  <h2 className="2xl:text-[18px] max-sm:text-[12px] text-white line-clamp mt-6">
                    {e.snippet.description.slice(0, 150)} ...
                  </h2>
                  ...
                  <div className="flex items-center justify-between mt-5">
                    ...
                    <button
                      className={`${sub}   py-2 rounded-lg px-7`}
                      onClick={() => setsub("bg-[#e2e2e2] text-[#000]")}
                    >
                      Subscribe
                    </button>
                  </div>
                  <div className="flex gap-3 mt-3 line-clamp-1 max-sm:max-w-[250px]"></div>
                  <hr className="opacity-30 mt-5" />
                  <div className="flex items-center gap-3 mt-4 whitespace-nowrap ">
                    ...
                    >
                      ...
                      <i
                        className={`fa-sharp fa-solid fa-thumbs-up ${like}`}
                      ></i>
                    </button>
                    <button onClick={() => setcomment("text-[#AAAAAA]")}>
                      <span className="text-[#AAAAAA] flex items-center gap-3">
                        {parseInt(e.statistics.commentCount).toLocaleString()}{" "}
                        <i className={`fa-solid fa-comment ${comment} `}></i>
                      </span>
                    </button>
                  </div>
                  <div className="mt-6">
